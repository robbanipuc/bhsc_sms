...
<table class="upper_table">
             <tr>
                ...
                বিষয়: '.subject_name_bangla($subject).'<br>
                শ্রেণি: '.$class.'<br>
...
$data.='
    ...
    <thead>
        <tr>
            <th>রোল</th><th>সৃজনশীল</th><th>বহুনির্বাচনী</th><th>ব্যবহারিক</th><th>মোট প্রাপ্ত নম্বর</th><th>মোট প্রাপ্ত নম্বর (৯০%)</th><th>শ্রেণি অভীক্ষা</th><th>সর্বমোট <br>প্রাপ্ত নম্বর</th><th>জিপি</th><th>লেটার গ্রেড</th><th>মন্তব্য</th>
        </tr>
    </thead>
        '
        ;
...
if($subject=='english'){
    $data.='
        <tr>
        <td>'.bfn($student_id).'</td><td>'.bfn($table_content[$subject.' total']).'</td><td></td><td></td><td>'.bfn($table_content[$subject.' total']).'</td><td>'.bfn($table_content[$subject.' total90']).'</td><td>'.bfn($table_content[$subject.' ct']).'</td><td>'.bfn($table_content[$subject.' total final']).'</td><td>'.bfn(number_format($table_content[$subject.' gp'],2)).'</td><td style="color:'.redmark($table_content[$subject.' lg']).';">'.lg_bangla($table_content[$subject.' lg']).'</td><td> </td>
        </tr>
';
}elseif($subject=='physics'||$subject=='chemistry'||$subject=='math'||$subject=='biology'||$subject=='ict'){
    $data.='
        <tr>
        <td>'.bfn($student_id).'</td><td>'.bfn($table_content[$subject.' crtv']).'</td><td>'.bfn($table_content[$subject.' mcq']).'</td><td>'.bfn($table_content[$subject.' prac']).'</td><td>'.bfn($table_content[$subject.' total']).'</td><td>'.bfn($table_content[$subject.' total90']).'</td><td>'.bfn($table_content[$subject.' ct']).'</td><td>'.bfn($table_content[$subject.' total final']).'</td><td>'.bfn(number_format($table_content[$subject.' gp'],2)).'</td><td style="color:'.redmark($table_content[$subject.' lg']).';">'.lg_bangla($table_content[$subject.' lg']).'</td><td> </td>
        </tr>
'; 
}else{
    if($exam_name=='pre_test' && $subject=='bangla'){
        $data.='
        <tr>
        <td>'.bfn($student_id).'</td><td>'.bfn($table_content[$subject.' total']).'</td><td></td><td></td><td></td><td>'.bfn($table_content[$subject.' total90']).'</td><td>'.bfn($table_content[$subject.' ct']).'</td><td>'.bfn($table_content[$subject.' total final']).'</td><td>'.bfn(number_format($table_content[$subject.' gp'],2)).'</td><td style="color:'.redmark($table_content[$subject.' lg']).';">'.lg_bangla($table_content[$subject.' lg']).'</td><td> </td>
        </tr>
'; 
        ...
        $data.='
        <tr>
        <td>'.bfn($student_id).'</td><td>'.bfn($table_content[$subject.' crtv']).'</td><td>'.bfn($table_content[$subject.' mcq']).'</td><td></td><td>'.bfn($table_content[$subject.' total']).'</td><td>'.bfn($table_content[$subject.' total90']).'</td><td>'.bfn($table_content[$subject.' ct']).'</td><td>'.bfn($table_content[$subject.' total final']).'</td><td>'.bfn(number_format($table_content[$subject.' gp'],2)).'</td><td style="color:'.redmark($table_content[$subject.' lg']).';">'.lg_bangla($table_content[$subject.' lg']).'</td><td> </td>
        </tr>
'; 
    }
}
