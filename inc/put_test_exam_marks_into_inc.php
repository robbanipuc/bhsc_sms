...
include('conn.php');
 ...
 while($table_content = mysqli_fetch_array($exam_mark_info)):
     ...
     if($subject=='physics' || $subject=='chemistry' || $subject=='biology' || $subject=='math')
     {
      ...
      $total_creative=$mark_creative1+$mark_creative2;
...
echo $exam_mark_table;
    ...
    if ($table_content_gp) {
    $total_mark =
        ($table_content_gp['bangla1 total'] ?? 0) +
        ($table_content_gp['bangla2 total'] ?? 0) +
        ($table_content_gp['english1'] ?? 0) +
        ($table_content_gp['english2'] ?? 0) +
        ($table_content_gp['ict total'] ?? 0) +
        ($table_content_gp[$subject1_1_marks] ?? 0) +
        ($table_content_gp[$subject1_2_marks] ?? 0) +
        ($table_content_gp[$subject2_1_marks] ?? 0) +
        ($table_content_gp[$subject2_2_marks] ?? 0) +
        ($table_content_gp[$subject3_1_marks] ?? 0) +
        ($table_content_gp[$subject3_2_marks] ?? 0) +
        ($table_content_gp[$fourth_sub_1_marks] ?? 0) +
        ($table_content_gp[$fourth_sub_2_marks] ?? 0);
} else {
    $total_mark = 0; // fallback if no data
}
      ...
      if ($table_content_gp) {
    $total_gp =
        ($table_content_gp['bangla gp'] ?? 0) +
        ($table_content_gp['english gp'] ?? 0) +
        ($table_content_gp['ict gp'] ?? 0) +
        ($table_content_gp[$subject1_gp_col] ?? 0) +
        ($table_content_gp[$subject2_gp_col] ?? 0) +
        ($table_content_gp[$subject3_gp_col] ?? 0) +
        ($table_content_gp[$fourth_subject_gp_col] ?? 0);
} else {
    $total_gp = 0; // fallback if query returned no data
}
...
// Ensure $table_content_gp is not null
if (!$table_content_gp) {
    $grade = 'F';
    $gpa_without_4th = 0;
    $gpa = 0;
} 
// Check if any GP is 0
elseif (
    ($table_content_gp['bangla gp'] ?? 0) == 0 ||
    ($table_content_gp['english gp'] ?? 0) == 0 ||
    ($table_content_gp['ict gp'] ?? 0) == 0 ||
    ($table_content_gp[$subject1_gp_col] ?? 0) == 0 ||
    ($table_content_gp[$subject2_gp_col] ?? 0) == 0 ||
    ($table_content_gp[$subject3_gp_col] ?? 0) == 0
) {
    $grade = 'F';
    $gpa_without_4th = 0;
    $gpa = 0;
} else {
    // Now $gpa is safe to use
    if ($gpa == 5) {
        $grade = 'A+';
    } elseif ($gpa >= 4 && $gpa < 5) {
        $grade = 'A';
...
// Ensure $table_content_position_gp is not null
$gpa = $table_content_position_gp['gpa'] ?? 0;

if ($gpa == 0) {
    $bangla      = ($table_content_position_gp['bangla gp'] ?? 0) == 0 ? 1 : 0;
    $english     = ($table_content_position_gp['english gp'] ?? 0) == 0 ? 1 : 0;
    $ict         = ($table_content_position_gp['ict gp'] ?? 0) == 0 ? 1 : 0;
    $subject1    = ($table_content_position_gp[$subject1_gp_col] ?? 0) == 0 ? 1 : 0;
    $subject2    = ($table_content_position_gp[$subject2_gp_col] ?? 0) == 0 ? 1 : 0;
    $subject3    = ($table_content_position_gp[$subject3_gp_col] ?? 0) == 0 ? 1 : 0;
    $fourth_sub  = ($table_content_position_gp[$fourth_subject_gp_col] ?? 0) == 0 ? 1 : 0;
